...
static bool IsSpikeyControlCollection(const HIDP_CAPS* caps)
{
    return (caps->UsagePage == CONTROL_COLLECTION_USAGE_PAGE &&
        caps->Usage == CONTROL_COLLECTION_USAGE &&
        caps->FeatureReportByteLength == THESPIKEYDRIVER_FEATURE_REPORT_SIZE_CB);
}
    ...
    for (wchar_t* p = list.data(); *p; p += wcslen(p) + 1) {
        ...
        if (QueryHidIdentity(h, &attr, &caps)) {
            wprintf(L"path=%s VID=0x%04X PID=0x%04X UsagePage=0x%04X Usage=0x%04X FeatureBytes=%u%s\n",
                ...
                IsSpikeyControlCollection(&caps) ? L" [spikey-control]" : L"");
        }
        CloseHandle(h);
    }
    return true;
...
static bool FindControlDevice(HANDLE* outHandle)
{
    ...
    std::vector<std::wstring> allSpikey;
    ...
    for (wchar_t* cur = list.data(); *cur; cur += wcslen(cur) + 1) {
        ...
        CloseHandle(h);
        if (!ok || !IsSpikeyControlCollection(&caps)) continue;
        allSpikey.push_back(cur);
        if (attr.VendorID == 0xDEED && attr.ProductID == 0xFEED) candidates.push_back(cur);
    }

    const std::vector<std::wstring>& target = !candidates.empty() ? candidates : allSpikey;
    if (target.empty()) {
        fprintf(stderr, "No Spikey control collection found.\n");
        return false;
    }
    if (target.size() > 1) {
        fprintf(stderr, "ERROR: duplicate Spikey control collections found (%zu). Clean stale Spikey devices/packages.\n", target.size());
...
{
    THESPIKEYDRIVER_FEATURE_REPORT packet = {};
    packet.ReportId = CONTROL_COLLECTION_REPORT_ID;
    packet.ControlCode = controlCode;
    packet.Version = THESPIKEYDRIVER_FEATURE_VERSION;
    packet.Payload.KeyEvent.StructSize = sizeof(packet.Payload.KeyEvent);
    packet.Payload.KeyEvent.Version = THESPIKEYDRIVER_CONTROL_VERSION;
...
static bool SendReleaseAll(HANDLE file)
{
    THESPIKEYDRIVER_FEATURE_REPORT packet = {};
    packet.ReportId = CONTROL_COLLECTION_REPORT_ID;
    packet.ControlCode = THESPIKEYDRIVER_FEATURE_RELEASE_ALL;
    packet.Version = THESPIKEYDRIVER_FEATURE_VERSION;
    if (!HidD_SetFeature(file, &packet, sizeof(packet))) {
...
static bool GetDriverStats(HANDLE file)
{
    THESPIKEYDRIVER_FEATURE_REPORT packet = {};
    packet.ReportId = CONTROL_COLLECTION_REPORT_ID;
    packet.ControlCode = THESPIKEYDRIVER_FEATURE_GET_STATS;
    packet.Version = THESPIKEYDRIVER_FEATURE_VERSION;
    if (!HidD_GetFeature(file, &packet, sizeof(packet))) {
        PrintWin32Error("HidD_GetFeature(GET_STATS)");
        return false;
    }
    const THESPIKEYDRIVER_STATS_OUTPUT& s = packet.Payload.Stats;
...
int wmain(int argc, wchar_t** argv)
{
    ...
    if (argc > 1 && wcscmp(argv[1], L"--stats") == 0) {
        ...
        UCHAR usage = 0; ok = ParseByteArg(argv[2], &usage) && SendKeyEvent(control, THESPIKEYDRIVER_FEATURE_KEY_DOWN, usage, 0);
    } else if (argc > 2 && wcscmp(argv[1], L"--up") == 0) {
        UCHAR usage = 0; ok = ParseByteArg(argv[2], &usage) && SendKeyEvent(control, THESPIKEYDRIVER_FEATURE_KEY_UP, usage, 0);
        ...
        ok = ParseByteArg(argv[2], &usage) &&
            SendKeyEvent(control, THESPIKEYDRIVER_FEATURE_KEY_DOWN, usage, 0) &&
            (Sleep(durationMs), true) &&
            SendKeyEvent(control, THESPIKEYDRIVER_FEATURE_KEY_UP, usage, 0);
