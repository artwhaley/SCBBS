/*++ Manual queue completion for SCMFD_Joystick_Root. --*/
...
VOID SCMFD_Joystick_RootKickManualQueue(_In_ PDEVICE_CONTEXT DeviceContext, _In_ ULONG DelayMs)
{
...
NTSTATUS SCMFD_Joystick_RootManualQueueInitialize(_In_ WDFDEVICE Device, _Out_ WDFQUEUE *Queue)
{
    NTSTATUS status;
    ...
    WDF_TIMER_CONFIG_INIT(&timerConfig, SCMFD_Joystick_RootEvtTimerFunc);
...
VOID SCMFD_Joystick_RootEvtTimerFunc(_In_ WDFTIMER Timer)
{
    NTSTATUS status;
    WDFQUEUE queue;
    PMANUAL_QUEUE_CONTEXT queueContext;
