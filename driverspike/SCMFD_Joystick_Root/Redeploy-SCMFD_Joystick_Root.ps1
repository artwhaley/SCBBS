...
<#
.SYNOPSIS
  Rebuild the SCMFD joystick driver and test client, then remove old installs and stage + install SCMFD_Joystick_Root.
...
. (Join-Path $ProjectDir 'SCMFD_Joystick_Root.InfHelpers.ps1')
...
function Invoke-DriverspikeRebuild {
    ...
    $projects = @(
        (Join-Path $solutionDir 'SCMFD_Joystick_Root\SCMFD_Joystick_Root.vcxproj'),
...
function Remove-SCMFD_Joystick_RootPnPDevices {
...
function Remove-SCMFD_Joystick_RootFromDriverStore {
    $lines = @()
    try {
...
function Resolve-SCMFD_Joystick_RootPackage {
    param(
        [Parameter(Mandatory)][string]$ProjectDir,
        [Parameter(Mandatory)][string]$Configuration
    )
    ...
    $outDirs = @(
        (Join-Path $sln "x64\$Configuration\SCMFD_Joystick_Root"),
        (Join-Path $sln "x64\$Configuration"),
        (Join-Path $ProjectDir "x64\$Configuration"),
        (Join-Path $ProjectDir "x64\$Configuration\SCMFD_Joystick_Root")
    ) | Where-Object { Test-Path -LiteralPath $_ }

    $packages = @{}
    foreach ($od in $outDirs) {
        foreach ($inf in (Get-SCMFD_Joystick_RootInfCandidates -OutDir $od -ProjectDir $ProjectDir -Configuration $Configuration)) {
            if ((Split-Path -Leaf $inf) -ne 'SCMFD_Joystick_Root.inf') { continue }
            $d = [IO.Path]::GetFullPath((Split-Path -Parent $inf))
            $cat = Join-Path $d 'SCMFD_Joystick_Root.cat'
            $dll = Join-Path $d 'SCMFD_Joystick_Root.dll'
    ...
    $preferred = [IO.Path]::GetFullPath((Join-Path $sln "x64\$Configuration\SCMFD_Joystick_Root"))
...
# --- 2) Package + deploy ---
$pkg = Resolve-SCMFD_Joystick_RootPackage -ProjectDir $ProjectDir -Configuration $Configuration
if (-not $pkg) {
    Write-Error @"
Could not find a complete package (SCMFD_Joystick_Root.inf + .cat + .dll).
...
Checked under:
  $([IO.Path]::GetFullPath((Join-Path (Split-Path -Parent $ProjectDir) "x64\$Configuration\SCMFD_Joystick_Root")))
...
if (-not $PSCmdlet.ShouldProcess($inf, 'Remove old devices, refresh driver store, install SCMFD_Joystick_Root')) {
    exit 0
}
...
Remove-SCMFD_Joystick_RootPnPDevices -DevconPath $devcon
Remove-SCMFD_Joystick_RootFromDriverStore
