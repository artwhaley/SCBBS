/*++ Device creation for SCMFD_Joystick_B_Root. --*/

#include "driver.h"
#include "device.tmh"

static volatile LONG g_SCMFD_Joystick_B_RootDeviceOrdinal;

static const WCHAR g_Product[] = L"SCMFD Joystick B";
static const WCHAR g_Serial[] = L"SCMFD Joystick B 0001";

EVT_WDF_DEVICE_FILE_CREATE SCMFD_Joystick_B_RootEvtDeviceFileCreate;
EVT_WDF_FILE_CLOSE SCMFD_Joystick_B_RootEvtFileClose;
EVT_WDF_FILE_CLEANUP SCMFD_Joystick_B_RootEvtFileCleanup;

VOID SCMFD_Joystick_B_RootEvtDeviceFileCreate(
...
VOID SCMFD_Joystick_B_RootEvtFileCleanup(_In_ WDFFILEOBJECT FileObject)
{
    UNREFERENCED_PARAMETER(FileObject);
}

VOID SCMFD_Joystick_B_RootEvtFileClose(_In_ WDFFILEOBJECT FileObject)
{
    UNREFERENCED_PARAMETER(FileObject);
}

NTSTATUS SCMFD_Joystick_B_RootCreateDevice(_Inout_ PWDFDEVICE_INIT DeviceInit)
{
    ...
    WDF_FILEOBJECT_CONFIG_INIT(
        &fileConfig,
        SCMFD_Joystick_B_RootEvtDeviceFileCreate,
        SCMFD_Joystick_B_RootEvtFileClose,
        SCMFD_Joystick_B_RootEvtFileCleanup);
    ...
    deviceContext->DebugInstanceOrdinal = (ULONG)InterlockedIncrement(&g_SCMFD_Joystick_B_RootDeviceOrdinal);
    deviceContext->JoystickSlot = 1u;
    ...
    hidAttributes->ProductID = SCMFD_JOYSTICK_ROOT_PID_B;
    hidAttributes->VersionNumber = HIDMINI_VERSION;

    status = SCMFD_Joystick_B_RootQueueInitialize(device);
    if (!NT_SUCCESS(status)) {
        return status;
    }

    status = SCMFD_Joystick_B_RootManualQueueInitialize(device, &deviceContext->ManualQueue);
    if (!NT_SUCCESS(status)) {
        return status;
    }
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,
        "SCMFD Joystick B created ordinal=%u VID=0x%x PID=0x%x reportSize=%u",
        deviceContext->DebugInstanceOrdinal,
        HIDMINI_VID,
        SCMFD_JOYSTICK_ROOT_PID_B,
