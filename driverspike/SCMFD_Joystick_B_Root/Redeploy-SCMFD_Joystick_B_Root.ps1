...
<#
.SYNOPSIS
  Rebuild the SCMFD joystick driver and test client, then remove old installs and stage + install SCMFD_Joystick_B_Root.
...
. (Join-Path $ProjectDir 'SCMFD_Joystick_B_Root.InfHelpers.ps1')
...
function Invoke-DriverspikeRebuild {
    ...
    $projects = @(
        (Join-Path $solutionDir 'SCMFD_Joystick_B_Root\SCMFD_Joystick_B_Root.vcxproj'),
...
function Remove-SCMFD_Joystick_B_RootPnPDevices {
    ...
    foreach ($args in @(            @('remove', 'root\SCMFD_Joystick_B_Root'),
            @('remove', '@root\SCMFD_Joystick_B_Root'),
            @('remove', 'root\SCMFD_Joystick_B_Root'),
            @('remove', 'root\SCMFD_Joystick_B_Root'),
            @('remove', '@root\SCMFD_Joystick_B_Root'),
            @('remove', 'ROOT\SCMFD_Joystick_B_Root')
...
function Remove-SCMFD_Joystick_B_RootFromDriverStore {
    $lines = @()
    try {
    ...
    $toDelete = $packages | Where-Object {
        ...
        ($null -ne $_.Provider -and $_.Provider -match '(?i)scmfd_joystick_b_root')
    }
...
function Resolve-SCMFD_Joystick_B_RootPackage {
    param(
        [Parameter(Mandatory)][string]$ProjectDir,
        [Parameter(Mandatory)][string]$Configuration
    )
    ...
    $outDirs = @(
        (Join-Path $sln "x64\$Configuration\SCMFD_Joystick_B_Root"),
        (Join-Path $sln "x64\$Configuration"),
        (Join-Path $ProjectDir "x64\$Configuration"),
        (Join-Path $ProjectDir "x64\$Configuration\SCMFD_Joystick_B_Root")
    ) | Where-Object { Test-Path -LiteralPath $_ }

    $packages = @{}
    foreach ($od in $outDirs) {
        foreach ($inf in (Get-SCMFD_Joystick_B_RootInfCandidates -OutDir $od -ProjectDir $ProjectDir -Configuration $Configuration)) {
            if ((Split-Path -Leaf $inf) -ne 'SCMFD_Joystick_B_Root.inf') { continue }
            $d = [IO.Path]::GetFullPath((Split-Path -Parent $inf))
            $cat = Join-Path $d 'SCMFD_Joystick_B_Root.cat'
            $dll = Join-Path $d 'SCMFD_Joystick_B_Root.dll'
    ...
    $preferred = [IO.Path]::GetFullPath((Join-Path $sln "x64\$Configuration\SCMFD_Joystick_B_Root"))
...
# --- 2) Package + deploy ---
$pkg = Resolve-SCMFD_Joystick_B_RootPackage -ProjectDir $ProjectDir -Configuration $Configuration
if (-not $pkg) {
    Write-Error @"
Could not find a complete package (SCMFD_Joystick_B_Root.inf + .cat + .dll).
...
Checked under:
  $([IO.Path]::GetFullPath((Join-Path (Split-Path -Parent $ProjectDir) "x64\$Configuration\SCMFD_Joystick_B_Root")))
...
if (-not $PSCmdlet.ShouldProcess($inf, 'Remove old devices, refresh driver store, install SCMFD_Joystick_B_Root')) {
    exit 0
}
...
Remove-SCMFD_Joystick_B_RootPnPDevices -DevconPath $devcon
Remove-SCMFD_Joystick_B_RootFromDriverStore

Write-Host "pnputil /add-driver `"$inf`" /install"
& pnputil.exe /add-driver $inf /install 2>&1 | ForEach-Object { Write-Host $_ }

if ($devcon) {
    Write-Host "devcon install `"$inf`" root\SCMFD_Joystick_B_Root"
    Invoke-Devcon -DevconPath $devcon -Arguments @('install', $inf, 'root\SCMFD_Joystick_B_Root')
}

Write-Host 'Redeploy finished. If the driver still fails, check %windir%\INF\setupapi.dev.log for the last SCMFD_Joystick_B_Root section.'
