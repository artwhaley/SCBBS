/*++ Manual queue completion for SCMFD_Joystick_A_Root. --*/
...
NTSTATUS SCMFD_Joystick_A_RootManualQueueInitialize(_In_ WDFDEVICE Device, _Out_ WDFQUEUE *Queue)
{
    NTSTATUS status;
    ...
    WDF_TIMER_CONFIG_INIT(&timerConfig, SCMFD_Joystick_A_RootEvtTimerFunc);
...
VOID SCMFD_Joystick_A_RootEvtTimerFunc(_In_ WDFTIMER Timer)
{
    NTSTATUS status;
    WDFQUEUE queue;
    PMANUAL_QUEUE_CONTEXT queueContext;
