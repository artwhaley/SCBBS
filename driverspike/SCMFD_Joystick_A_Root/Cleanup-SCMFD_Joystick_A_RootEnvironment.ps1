...
function Remove-SCMFD_Joystick_A_RootWinmmCache {
    $paths = @(
...
$devices = @(Get-PnpDevice -ErrorAction SilentlyContinue | Where-Object {
    ...
    ($_.FriendlyName -like '*SCMFD Joystick A*' -and $_.InstanceId -like 'ROOT\HIDCLASS\*')
})

$pkgs = @(Get-PnputilDriverPackages | Where-Object {
    ($_.OriginalName -match '(?i)^scmfd_joystick_a_root\.inf$') -or
    ($_.OriginalName -match '(?i)^scmfd_joystick_root\.inf$') -or
    ($_.Provider -match '(?i)scmfd_joystick_a_root') -or
...
Remove-SCMFD_Joystick_A_RootWinmmCache
