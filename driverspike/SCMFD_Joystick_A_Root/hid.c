/*++ Minimal joystick HID descriptor + IOCTL dispatch for SCMFD_Joystick_A_Root. --*/
...
NTSTATUS SetFeature(_In_ PQUEUE_CONTEXT QueueContext, _In_ WDFREQUEST Request)
{
    ...
    if (NT_SUCCESS(status)) {
        SCMFD_Joystick_A_RootKickManualQueue(QueueContext->DeviceContext, 1);
    }
    WdfRequestSetInformation(Request, 0);
    return status;
}

VOID SCMFD_Joystick_A_RootEvtIoHidDeviceControl(
    _In_ WDFQUEUE Queue,
